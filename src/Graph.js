...
export default function Graph({ dailyData, activeLocation }) {
  ...
  useEffect(() => {
    ...
    const xAxis = d3.axisBottom(xScale).tickFormat((index) => index)

    d3.select('.x-axis').style('transform', 'translateY(150px)').call(xAxis)

    // Y-axis
    const yScale = d3.scaleLinear().domain([0, yMax]).range([150, 0])
    ...
    svg.append('path').data([lineData]).attr('class', 'dead-line').attr('fill', 'none').attr('stroke', 'gray').attr('stroke-width', '2px').attr('d', deadLine)
